...
import { mediaQuery, createMediaMutation, updateMediaMutation, deleteMediaMutation } from './media.graphql';
...
export class MediaService {
  ...
  create(media: Partial<Media>) {
    ...
    res.data.insert_media.returning[0]))
  }
  ...
  update(media: Partial<Media>) {
    ...
    res.data.update_media.returning[0]))
  }
  ...
  delete(media: Partial<Media>) {
    return this.apollo.mutate({
      ...
      res.data.delete_media.returning[0]))
  }
}
