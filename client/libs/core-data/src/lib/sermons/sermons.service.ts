...
export class SermonsService {
  ...
  create(sermon: Partial<Sermon>) {
    ...
    return this.apollo.mutate({
      ...
      map((response: ApolloQueryResult<any>) => response.data.insert_sermons.returning[0])
    );
  }
  ...
  update(sermon: Partial<Sermon>) {
    ...
    return this.apollo.mutate({
      ...
      map((response: ApolloQueryResult<any>) => response.data.update_sermons.returning[0])
    );
  }
  ...
  delete(sermon: Partial<Sermon>) {
    return this.apollo.mutate({
      ...
      map((response: ApolloQueryResult<any>) => response.data.delete_sermons.returning[0])
    );
  }
}
